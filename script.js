...
function updateNutritionUI() {
  ...
  meals.forEach(meal => {
    ...
    if (card) {
      ...
      const mealName = nameEl ? nameEl.textContent.trim() : 'Meal';
      ...
      if (isDone) {
        ...
        if (button) {
          ...
          button.setAttribute('aria-label', `Unmark ${mealName} as done`);
        }
        completed++;
      } else {
        ...
        if (button) {
          ...
          button.setAttribute('aria-label', `Mark ${mealName} as done`);
        }
      }
    }
  });
